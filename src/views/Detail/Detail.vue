            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  :value="skuNum"
                  class="itxt"
                  @change="handleChangeSkuNum"
                />
                <a href="javascript:" class="plus" @click="addOne">+</a>
                <a href="javascript:" class="mins" @click="minusOne">-</a>
              </div>
              <div class="add">
                <a @click="addToCartShop">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...mapGetters(["categoryView", "skuInfo", "price", "spuSaleAttrList"]),
    skuImageList() {
  ...
  methods: {
    ...
    handleChangeSkuNum(event) {
      let num = event.target.value * 1;
      if (Number.isInteger(num) && this.isValid(num)) {
        this.skuNum = num;
      } else {
    ...
    isValid(n) {
      return n > 0 && n <= this.max;
    },
    async addToCartShop() {
      try {
        await this.$store.dispatch("addToCartShop", {
        ...
        // 跳转到成功添加购物车页面
        let location = { path: "/addcartsuccess", query: {skuNum: this.skuNum} };
