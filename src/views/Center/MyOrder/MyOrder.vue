    ...
    <div class="order-content">
      ...
      <div class="orders">
        <table class="order-item" v-for="record in records" :key="record.id">
          ...
          <tbody>
            <tr v-for="(item, index) in record.orderDetailList" :key="index">
              <td width="60%">
                <div class="typographic">
                  <img :src="item.imgUrl" style="width: 82px; height: 82px" />
...
export default {
  ...
  computed: {
    ...
    pageInfo() {
        return {pageNo:this.orderInfo.current, pageSize:this.orderInfo.size, total:this.orderInfo.total, totalPages:this.orderInfo.pages};
    }
  },
  methods: {
    async getOrderList(pageNo = 1, pageSize = 5) {
      // 获取我的订单列表
      let result = await this.$API.requestOrderList(pageNo, pageSize);
      if (result.status == 200 && result.data.code == 200) {
        console.log("myorder", result.data.data);
