...
<script>
// import { requestPayInfo } from '@/api';
...
export default {
  ...
  methods: {
    async getPayInfo() {
      let result = await this.$API.requestPayInfo(this.$route.query.tradeNo);
    ...
    async open() {
      ...
      this.$alert(`<img src=${qrcode} />`, "请使用微信支付", {
        center: true,
      ...
      // 设置一个定时任务，用于每隔一段时间向服务器请求订单支付状态信息
      this.timer = setInterval(async ()=>{
        let result = await this.$API.requestPayStatus(this.payInfo.orderId);
        console.log('payStatus', result);
        console.log('payStatus data', result.data);
        if(result.status == 200 && result.data.code == 200) {
          console.log('支付成功');
          // 保存付款状态码
